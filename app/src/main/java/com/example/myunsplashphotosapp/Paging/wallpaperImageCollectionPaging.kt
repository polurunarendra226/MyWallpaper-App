...
class wallpaperImageCollectionPaging(var api:api_service):PagingSource<Int, User5Item>() {

    override fun getRefreshKey(state: PagingState<Int, User5Item>): Int? {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, User5Item> {
       return try {
           var position = params.key?:1
           var result=api.getCollection(page = position, client_id = Utils.Api_Key)
